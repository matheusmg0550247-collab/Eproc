...
# ==============================================================
# WATCHER GLOBAL (carimbo de versão)
# - id 9999 na tabela app_state guarda {"v": <int>, "ts": "...", ...}
# - sempre que qualquer equipe salva, incrementa esse carimbo
# - o watcher dos clientes só observa esse id (barato) e reruna o app quando mudar
# ==============================================================
GLOBAL_WATCHER_ID = 9999

def get_global_state_version(sb=None) -> int:
    try:
        sb = sb or get_supabase()
        if not sb:
            return 0
        res = sb.table("app_state").select("data").eq("id", GLOBAL_WATCHER_ID).limit(1).execute()
        if not getattr(res, "data", None):
            return 0
        data = (res.data[0] or {}).get("data") or {}
        v = data.get("v") or data.get("state_version") or 0
        # aceita int ou float string
        s = str(v).strip()
        if s.isdigit():
            return int(s)
        try:
...
def bump_global_state_version(team_id=None, actor=None, reason=None, sb=None) -> None:
    try:
        sb = sb or get_supabase()
        if not sb:
            return
        ...
        payload = {
            "v": v,
            "ts": get_brazil_time().isoformat(),
            "team_id": team_id,
            "actor": actor,
            "reason": reason
        }
        sb.table("app_state").upsert({"id": GLOBAL_WATCHER_ID, "data": payload}).execute()
    except Exception:
        # watcher não pode derrubar o app
        pass
...
def save_state_to_db(app_id, state_data):
    ...
    try:
        ...
        # --- WATCHER: bump global version (qualquer equipe) ---
        bump_global_state_version(team_id=app_id, actor=st.session_state.get('consultor_logado'), reason='save_state', sb=sb)
...
def watcher_de_atualizacoes():
    """Watcher global: a cada 20s, consulta o carimbo de versão e reruna o app se mudou.
    Obs.: Não faz sync pesado dentro do fragment; só dispara o rerun para o ciclo normal puxar do banco.
    """
    try:
        # Evita brigar com o clique/salvamento local
        if time.time() < st.session_state.get('_skip_db_sync_until', 0):
            return
        ...
        sv = get_global_state_version(sb)
        last = st.session_state.get('_watch_last_sv', None)

        # primeira leitura: só registra
        if last is None:
            st.session_state['_watch_last_sv'] = sv
            return
...
def render_dashboard(team_id: int, team_name: str, consultores_list: list, webhook_key: str, app_url: str, other_team_id: int, other_team_name: str, usuario_logado: str):
    ...
    # Auto-refresh global (desativado; usando watcher global)
    ...
    def render_header_info_left():
        ...
        else: raw_ordered = [n for n in queue if n != responsavel] if responsavel else list(queue)
        lista_pularam = [n for n in queue if skips.get(n, False) and n != responsavel]
        demais_na_fila = [n for n in raw_ordered if (n != responsavel) and (n != proximo) and not skips.get(n, False)]
