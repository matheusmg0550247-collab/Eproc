...
def render_agenda_eproc_sidebar()
    ...
    st.caption(fHoje {dia_pt} — {now_br.strftime('%d%m%Y')})
    ...
    with st.expander(Ver visão semanal consolidada, expanded=False)
        for row in EPROC_VISAO_SEMANAL
            ...
            obs_html = fdiv style='font-size13px; margin-top6px; color#6b7280;'i{obs_text}idiv if obs_text else 
            
            st.markdown(
                f
div style='border{border}; background{bg}; padding10px 12px; border-radius12px; margin8px 0;'
  div style='font-weight800; font-size15px; margin-bottom6px;'🗓️ {row.get('dia','')}div
  div style='font-size14px; margin2px 0;'b🕘 Manhãb {row.get('manha','')}div
  div style='font-size14px; margin2px 0;'b🕜 Tardeb {row.get('tarde','')}div
  div style='font-size14px; margin2px 0;'b🎙️ Sessõesb {row.get('sessao','')}div
  {obs_html}
div
...
def _badge_ramal_html(ramal)
    if not ramal
        return ''
    return fspan style='margin-left8px; padding2px 8px; border-radius999px; border1px solid #ddd; font-size12px; background#f7f7f7;'☎ {ramal}span
...
APP_URL_CLOUD = 'httpscontrole-bastao-cesupe.streamlit.app'

# Secrets
# N8N  WhatsApp (Z-API via n8n)
# Preferencialmente configure em .streamlitsecrets.toml
# [n8n]
...
# - chat.registro (demais registrosferramentas)
...
def post_n8n(url str, payload dict) - bool
    Envia evento para n8n (silencioso). Retorna TrueFalse.
    if not url
        return False
    try
...
def render_operational_summary()
    ...
    st.markdown('🔗 Dados Semanais Cesupe — [Abrir no Looker Studio](httpslookerstudio.google.comreporting8adc4cf8-94f6-4333-b808-4a300151af09)', unsafe_allow_html=False)
...
def set_logmein_status(consultor, em_uso)
    ...
    except Exception as e st.error(fErro LogMeIn DB {e})
    ...
    try
        ...
        if 'hora_periodo' in dados
             if dados['hora_periodo']
                dados['motivo'] = f{dados.get('motivo', '')} - HoraPeríodo {dados['hora_periodo']}
        ...
        st.error(fErro ao salvar no Supabase {e})
        return False
...
def gerar_docx_certidao_internal(tipo, numero, data, consultor, motivo, chamado=, hora=, nome_parte=)
    try
        ...
        runner = head_p.add_run(TRIBUNAL DE JUSTIÇA DO ESTADO DE MINAS GERAISn)
        runner.bold = True
        head_p.add_run(Rua Ouro Preto, N° 1564 - Bairro Santo Agostinho - CEP 30170-041 - Belo Horizonte - MGnwww.tjmg.jus.br - Andar 3º e 4º PV)
        doc.add_paragraph(n)
        
        # Numeração e Assunto
        if tipo == 'Geral' p_num = doc.add_paragraph(fParecer GEJUDDIRTECTJMG nº ____2026. Assunto Notifica erro no “JPe – 2ª Instância” ao peticionar.)
        else p_num = doc.add_paragraph(fParecer Técnico GEJUDDIRTECTJMG nº ____2026. Assunto Notifica erro no “JPe – 2ª Instância” ao peticionar.)
        ...
        try
            ...
            data_extenso_str = fBelo Horizonte, {dt_obj.day} de {meses[dt_obj.month]} de {dt_obj.year}
        except
            data_extenso_str = fBelo Horizonte, {data} 
            
        doc.add_paragraph(data_extenso_str)
              
        doc.add_paragraph(fExmo(a). Senhor(a) Relator(a),)
        
        if tipo == 'Geral'
            corpo = doc.add_paragraph(); corpo.alignment = WD_ALIGN_PARAGRAPH.JUSTIFY
            txt = (fPara fins de cumprimento dos artigos 13 e 14 da Resolução nº 7802014 do Tribunal de Justiça do Estado de Minas Gerais, 
                   finformamos que em {data} houve indisponibilidade do portal JPe, superior a uma hora, {hora}, que impossibilitou o peticionamento eletrônico de recursos em processos que já tramitavam no sistema.)
            corpo.add_run(txt)
            doc.add_paragraph(nColocamo-nos à disposição para outras que se fizerem necessárias.)
            
        elif tipo in ['Eletrônica', 'Eletrônico']
            corpo = doc.add_paragraph(); corpo.alignment = WD_ALIGN_PARAGRAPH.JUSTIFY
            corpo.add_run(fInformamos que de {data}, houve indisponibilidade específica do sistema para o peticionamento do processo nº {numero})
            if nome_parte corpo.add_run(f, ParteAdvogado {nome_parte})
            corpo.add_run(.nn)
            corpo.add_run(fO Chamado de número {chamado if chamado else '_____'}, foi aberto e encaminhado à DIRTEC (Diretoria Executiva de Tecnologia da Informação e Comunicação).nn)
            ...
            corpo1.add_run(fInformamos que no dia {data}, houve indisponibilidade específica do sistema para o peticionamento do processo nº {numero})
            if nome_parte corpo1.add_run(f, ParteAdvogado {nome_parte})
            corpo1.add_run(.)
            
            corpo2 = doc.add_paragraph(); corpo2.alignment = WD_ALIGN_PARAGRAPH.JUSTIFY
            corpo2.add_run(fO Chamado de número {chamado if chamado else '_____'}, foi aberto e encaminhado à DIRTEC (Diretoria Executiva de Tecnologia da Informação e Comunicação).)
            
            corpo3 = doc.add_paragraph(); corpo3.alignment = WD_ALIGN_PARAGRAPH.JUSTIFY
            corpo3.add_run(Diante da indisponibilidade específica, não havendo um prazo para solução do problema, a Primeira Vice-Presidência recomenda o ingresso dos autos físicos, nos termos do § 2º, do artigo 14º, da Resolução nº 7802014, do Tribunal de Justiça do Estado de Minas Gerais.)
            
            doc.add_paragraph(Colocamo-nos à disposição para outras informações que se fizerem necessárias.)

        doc.add_paragraph(nRespeitosamente,)
        sign = doc.add_paragraph(n___________________________________nWaner Andrade Silvan0-009020-9nCoordenação de Análise e Integração de Sistemas Judiciais Informatizados - COJINnGerência de Sistemas Judiciais - GEJUDnDiretoria Executiva de Tecnologia da Informação e Comunicação - DIRTEC)
...
def send_horas_extras_to_chat(consultor, data, inicio, tempo, motivo)
    msg = f⏰ Registro de Horas Extrasnn👤 Consultor {consultor}n📅 Data {data.strftime('%d%m%Y')}n🕐 Início {inicio.strftime('%H%M')}n⏱️ Tempo Total {tempo}n📝 Motivo {motivo}
...
def send_atendimento_to_chat(consultor, data, usuario, nome_setor, sistema, descricao, canal, desfecho, jira_opcional=)
    jira_str = fn🔢 Jira CESUPE-{jira_opcional} if jira_opcional else 
    msg = f📋 Novo Registro de Atendimentonn👤 Consultor {consultor}n📅 Data {data.strftime('%d%m%Y')}n👥 Usuário {usuario}n🏢 NomeSetor {nome_setor}n💻 Sistema {sistema}n📝 Descrição {descricao}n📞 Canal {canal}n✅ Desfecho {desfecho}{jira_str}
...
def send_chamado_to_chat(consultor, texto)
    ...
    msg = f🆘 Rascunho de ChamadoJiran📅 Data {data_envio}nn👤 Autor {consultor}nn📝 Texton{texto}
...
def handle_erro_novidade_submission(consultor, titulo, objetivo, relato, resultado)
    data_envio = get_brazil_time().strftime(%d%m%Y %H%M)
    msg = f🐛 Novo Relato de ErroNovidaden📅 Data {data_envio}nn👤 Autor {consultor}n📌 Título {titulo}nn🎯 Objetivon{objetivo}nn🧪 Relaton{relato}nn🏁 Resultadon{resultado}
...
def handle_sugestao_submission(consultor, texto)
    ...
    msg = f💡 Nova Sugestãon📅 Data {data_envio}n👤 Autor {consultor}n🌐 IP {ip_usuario}nn📝 Sugestãon{texto}
    return notify_registro_ferramenta(H_EXTRAS, consultor, dados={data data.strftime(%d%m%Y), inicio inicio.strftime(%H%M), tempo tempo, motivo motivo}, mensagem=msg)
...
def save_state()
    ...
    except Exception as e print(fErro save {e})

def format_time_duration(duration)
    if not isinstance(duration, timedelta) return '------'
    s = int(duration.total_seconds()); h, s = divmod(s, 3600); m, s = divmod(s, 60)
    return f'{h02}{m02}{s02}'
...
def sync_state_from_db()
    ...
    except Exception as e print(fErro sync {e})
...
def check_and_assume_baton(forced_successor=None, immune_consultant=None)
    ...
    if target
        ...
        if 'Bastão' not in curr_s
            old_s = curr_s; new_s = fBastão  {old_s} if old_s and old_s != Indisponível else Bastão
...
def notify_bastao_giro(reason='update', actor=None)
    ...
    try
        ...
        msg_final = (
            f🔄 Troca de Bastão - {nome_equipe}nn
            f👤 Agora {holder}n
...
def notify_registro_ferramenta(tipo str, actor str, dados dict = None, mensagem str = None) - bool
    ...
    if not mensagem
        mensagem = fNovo registro {tipo} por {actor}
...
def rotate_bastao()
    ...
    if next_idx != -1
        ...
        new_n_status = fBastão  {old_n_status} if old_n_status else Bastão
...
def render_dashboard(team_id int, team_name str, consultores_list list, webhook_key str, app_url str, other_team_id int, other_team_name str, usuario_logado str)
    ...
    if st_autorefresh
        st_autorefresh(interval=20000, key=fautorefresh_{team_id})
    ...
    try
        ...
        st.markdown(fdiv class='sticky-topbar'div style='displayflex; justify-contentspace-between; align-itemscenter; gap12px;'divb👤 Logado comob {_user_top} &nbsp; span class='muted'span &nbsp; b👥 Equipeb {_team_top}divdiv class='muted'🕒 {_now_top}divdivdiv, unsafe_allow_html=True)
    except pass
    ...
    st.markdown(stylediv.stButton  button {width 100%; height 3rem;}style, unsafe_allow_html=True)
    st.components.v1.html(scriptwindow.scrollTo(0, 0);script, height=0)
    ...
    def render_header_info_left()
        ...
        with c_topo_esq
            img = get_img_as_base64_cached(PUG2026_FILENAME); src = fdataimagepng;base64,{img} if img else GIF_BASTAO_HOLDER
            st.markdown(fdiv style=display flex; align-items center; gap 15px;h1 style=margin 0; padding 0; font-size 2.2rem; color #FF8C00; text-shadow 1px 1px 2px #FF4500;Controle Bastão Cesupe 2026 {BASTAO_EMOJI}h1img src={src} style=width 150px; height 150px; border-radius 10px; border 4px solid #FF8C00; object-fit cover;div, unsafe_allow_html=True)
        with c_topo_dir
            ...
            st.caption(fID ...{st.session_state.get('device_id_val', '')[-4]})
        ...
        if responsavel
            st.markdown(fdiv style=background linear-gradient(135deg, #FFF3E0 0%, #FFFFFF 100%); border 3px solid #FF8C00; padding 25px; border-radius 15px; display flex; align-items center; box-shadow 0 4px 15px rgba(255, 140, 0, 0.3); margin-bottom 20px;div style=flex-shrink 0; margin-right 25px;img src={GIF_BASTAO_HOLDER} style=width 90px; height 90px; border-radius 50%; object-fit cover; border 2px solid #FF8C00;divdivspan style=font-size 14px; color #555; font-weight bold; text-transform uppercase; letter-spacing 1.5px;Atualmente comspanbrspan style=font-size 42px; font-weight 800; color #FF4500; line-height 1.1;{responsavel}spandivdiv, unsafe_allow_html=True)
        ...
        if proximo
            ...
            st.markdown(fPróximo Bastão {proximo}{ic}, unsafe_allow_html=True)
        ...
        if lista_pularam st.markdown(fConsultor(es) pulou(pularam) o bastão {', '.join(lista_pularam)})
...
,
        ...
        with st.expander('🧭 Painel (outra equipe  LogMeIn  trocar consultor)', expanded=False)
            ...
            if other_id
                ...
                if other_responsavel st.markdown(fCom o Bastão agora {_fmt_other(other_responsavel)}, unsafe_allow_html=True)
                else st.markdown(Com o Bastão agora _Ninguém_, unsafe_allow_html=True)
                if proximo_outro st.markdown(fPróximo Bastão {_fmt_other(proximo_outro)}, unsafe_allow_html=True)
    ...
    def render_status_list()
        ...
        if not render_order st.markdown('_Ninguém na fila._')
            ...
            for i, nome in enumerate(render_order, 1)  # fila começa em 1º lugar
                ...
                elif skip_flag display = f'strong{i}º {nome}{indic_icons}strong{extra} span style=background-color #FEF3C7; padding 2px 8px; border-radius 10px;Pulando ⏭️span'
                else display = f'strong{i}º {nome}{indic_icons}strong{extra} span style=background-color #FFEDD5; padding 2px 8px; border-radius 10px;Aguardandospan'
                ...
                for item in itens
                    ...
                    if titulo == 'Indisponível'
                        ...
                        if col_c.button('⬆️', key=f'btn_back_{titulo}_{nome}', help=Voltar para a fila)
                            ...
                            st.rerun()
                    indic_icons = _icons_telefone_cafe(st.session_state.get('quick_indicators', {}).get(nome, {}))
                    col_n.markdown(fdiv style='font-size 16px; margin 2px 0;'strong{nome}{indic_icons}strongspan style='background-color {bg_hex}; color #333; padding 2px 8px; border-radius 12px; font-size 14px; margin-left 8px;'{desc}spandiv, unsafe_allow_html=True)
            st.markdown('---')
    ...
    with col_principal
        ...
        with c_nome
             st.caption(fLogado como {st.session_state.get('consultor_logado', st.session_state.get('consultor_selectbox', ''))})
        ...
        with c_act1
            if st.button(🎭 EntrarSair Fila, use_container_width=True) toggle_presence_btn(); st.rerun()
        with c_act2
        ...
        c_t5.button(🐛 ErroNovidade, use_container_width=True, on_click=toggle_view, args=(erro_novidade,))
            ...
            with st.container(border=True)
                ...
                with c1
                    if st.button(Confirmar, type=primary, use_container_width=True) 
                        ...
                        status_msg = fAtividade {', '.join(at_t)} - {at_e}
            ...
            with st.container(border=True)
                ...
                with c_ok
                    if st.button('✅ Confirmar', type='primary', use_container_width=True)
                        ...
                        else st.session_state.active_view = None; update_status(fAtendimento Presencial {local_presencial.strip()} - {objetivo_presencial.strip()}, True); st.rerun() 
                with c_cancel
                    if st.button('❌ Cancelar', use_container_width=True) st.session_state.active_view = None; st.rerun()
                ...
                with c_ok
                    if st.button('✅ Confirmar', type='primary', use_container_width=True)
                        if not proj_nome.strip() st.warning('Digite o nome do projeto.')
                            ...
                            st.session_state.active_view = None
                            status_msg = fProjeto {proj_nome.strip()}
        ...
        if st.session_state.active_view == 'menu_treinamento'
            with st.container(border=True)
                st.subheader('🎓 Registrar Treinamento'); tema = st.text_input('TemaConteúdo'); obs = st.text_input('Observação (opcional)')
                c_ok, c_cancel = st.columns(2)
                with c_ok
                    if st.button('✅ Confirmar', type='primary', use_container_width=True)
                        if not tema.strip() st.warning('Informe o tema.')
                        else st.session_state.active_view = None; update_status(fTreinamento {tema.strip()} + (f - {obs.strip()} if obs.strip() else ), True); st.rerun()
                with c_cancel
                    if st.button('❌ Cancelar', use_container_width=True) st.session_state.active_view = None; st.rerun()
            ...
            with st.container(border=True)
                ...
                with c_ok
                    if st.button('✅ Confirmar', type='primary', use_container_width=True)
                        ...
                        else st.session_state.active_view = None; update_status(fReunião {assunto.strip()} + (f - {obs.strip()} if obs.strip() else ), True); st.rerun()
                with c_cancel
                    if st.button('❌ Cancelar', use_container_width=True) st.session_state.active_view = None; st.rerun()
                ...
                with c_ok
                    if st.button('✅ Confirmar', type='primary', use_container_width=True)
                        ...
                        if not consultor or consultor == 'Selecione um nome' st.error('Selecione um consultor.')
                            ...
                            st.session_state.active_view = None; update_status(fSessão {sessao_livre} + (f - {obs.strip()} if obs.strip() else ), True); st.rerun()
                with c_cancel
                    if st.button('❌ Cancelar', use_container_width=True) st.session_state.active_view = None; st.rerun()
        ...
        if st.session_state.active_view == erro_novidade
            with st.container(border=True)
                st.header(🐛 ErroNovidade); tit = st.text_input(Título); obj = st.text_area(Objetivo); rel = st.text_area(Relato); res = st.text_area(Resultado)
                if st.button(Enviar) 
        ...
        if st.session_state.active_view == certidao
            with st.container(border=True)
                ...
                c_data = st.date_input(Data do Evento, value=get_brazil_time().date(), format=DDMMYYYY)
                tipo_cert = st.selectbox(Tipo, [Física, Eletrônica, Geral])
                c_cons = st.session_state.consultor_selectbox
                c_hora = ; c_motivo = st.text_area(MotivoDetalhes, height=100)
            
                if tipo_cert == Geral 
                    c_hora = st.text_input(HorárioPeríodo (Ex 13h às 15h))
                    ...
                    c_chamado = c2.text_input(IncidenteChamado)
                    c3, c4 = st.columns(2)
                    c_nome_parte = c3.text_input(Nome da ParteAdvogado)
                    ...
                    if st.button(💾 Salvar e Notificar, type=primary, use_container_width=True)
                        if verificar_duplicidade_certidao(tipo_cert, c_proc, c_data) st.session_state.aviso_duplicidade = True
                            ...
                            if salvar_certidao_db(payload)
                                msg_cert = f🖨️ Nova Certidão Registradan👤 Autor {c_cons}n📅 Data {c_data.strftime('%d%m%Y')}n📄 Tipo {tipo_cert}n📂 Proc {c_proc}
