...
URL_GOOGLE_SHEETS = "https://script.google.com/macros/s/AKfycby4gt0lWWWGs2jasJ7tBpVVCABq8RUNwggxvMHqRqv55SwMOAnXzn7xFN5S_vKiS3envg/exec"
...
OPCOES_ATIVIDADES = ["HP", "E-mail", "Whatsapp/Plantão", "Treinamento", "Homologação", "Redação Documentos", "Reunião", "Outros"]
ATIVIDADES_DETALHE = ["Treinamento", "Homologação", "Redação Documentos", "Outros"]

# ============================================
# 2. SISTEMA DE LOGS E GOOGLE SHEETS
# ============================================

def log_to_google_sheets(consultor, status_antigo, status_novo, duracao):
    """Envia dados para o Google Sheets em segundo plano"""
    payload = {
        "consultor": consultor,
...
def registrar_mudanca(nome, novo_status):
    """Lógica central para mudar status e logar"""
    ...
    st.session_state.daily_logs.append({'c': nome, 'old': old_status, 'new': novo_status, 'dur': duracao})
...
# ============================================
# 3. PERSISTÊNCIA E LOGICA DE BASTÃO
# ============================================
...
# Auto Refresh
st_autorefresh(interval=8000, key="auto_refresh_global")
...
with c_esq:
    st.markdown(f'''<div style="display: flex; align-items: center; gap: 20px;">
        ...
        <img src="{GIF_BASTAO_HOLDER}" style="width: 100px; height: 100px; border-radius: 50%; border: 3px solid #FFD700; object-fit: cover;">
    </div>''', unsafe_allow_html=True)

with c_dir:
    # PAINEL ASSUMIR BASTÃO (EXATAMENTE IGUAL AO PRIMEIRO CÓDIGO)
...
# Lógica de GIFs Temporários
rot_time = st.session_state.get('rotation_gif_start_time')
if rot_time and (datetime.now() - rot_time).total_seconds() < 15:
...
with col_m:
    ...
    if dono:
        st.markdown(f'''<div style="background: #FFF8DC; border: 4px solid #FFD700; padding: 25px; border-radius: 20px; display: flex; align-items: center;">
            ...
            <span style="font-size: 38px; font-weight: bold; color: #000080;">{dono}</span>
        </div>''', unsafe_allow_html=True)
        dur_b = datetime.now() - (st.session_state.bastao_start_time or datetime.now())
        st.caption(f"⏱️ Tempo: {format_dur(dur_b)}")
    else: st.warning("Ninguém com o bastão. Marque o seu nome na lista ao lado!")
    ...
    if btns[0].button("🎯 Passar", use_container_width=True):
        ...
        if sel == dono:
            registrar_mudanca(sel, "") # Volta para disponível
            st.session_state.rotation_gif_start_time = datetime.now()
            check_and_assume_baton()
            st.rerun()
        else: st.error("Você não é o dono do bastão!")
    ...
    if st.session_state.active_view == "ses":
        with st.container(border=True):
            setor = st.text_input("Qual o Setor/Câmara?")
    ...
    st.markdown("---")
    # BOTÕES DE FERRAMENTAS (IMAGEM 2)
    t1, t2, t3, t4, t5 = st.columns(5)
    t1.button("📑 Checklist", use_container_width=True, on_click=lambda: set_v("chk"))
    t2.button("🆘 Chamados", use_container_width=True, on_click=lambda: set_v("cha"))
    t3.button("📝 Atendimento", use_container_width=True, on_click=lambda: set_v("reg"))
    t4.button("⏰ H. Extras", use_container_width=True, on_click=lambda: set_v("hex"))
    t5.button("🧠 Descanso", use_container_width=True, on_click=lambda: set_v("sim"))
...
with col_s:
    ...
    def render_section(label, items, color, is_tup=False):
        ...
        for i in items:
            ...
            c2.checkbox(" ", key=f"chk_{name}", value=(st.session_state.status_texto[name] in ['Bastão', '']), on_change=update_queue_callback, args=(name,), label_visibility="collapsed")
            if name == dono: c1.markdown(f"🥂 **{name}**")
