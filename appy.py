...
def carregar_dados_do_banco():
    ...
    try:
        ...
        # 3. Monta o DataFrame de Status Atual
        rows = []
...
def classificar_categoria(row):
    ...
    # 1. É o Dono do Bastão?
    if consultor == dono_bastao_real: return "Bastão"
    
    # 2. Está na Fila de Espera?
    # IMPORTANTE: Se está na fila, aparece na Fila, independente de ter atividade
...
# --- RENDERIZAÇÃO DAS COLUNAS (LÓGICA SENHA DE BANCO) ---
def renderizar_grid(df_sistema, cats):
    ...
    for i, (n_col, css) in enumerate(cats):
        with cols[i]:
            ...
            # --- ORDENAÇÃO E LÓGICA DE FILA ---
            proximo_nome = None # Variável para guardar quem é o "Próximo" oficial

            if n_col == "Bastão":
                if not filtrados.empty: filtrados = filtrados.sort_values('Data e Horário', ascending=False)
            
            elif n_col == "Fila Bastão":
                # ORDEM CRONOLÓGICA (Antiguidade)
                if not filtrados.empty: 
                    filtrados = filtrados.sort_values('Data e Horário', ascending=True)
                    
                    # CÁLCULO DO "PRÓXIMO" ELEGÍVEL
                    # Varre a lista ordenada. O primeiro que NÃO estiver pulando é o Próximo.
            ...
            if filtrados.empty:
                ...
                for j, (idx_df, row) in enumerate(filtrados.iterrows()):
                    ...
                    status_full = row['Status Atual']
                    sis_icon = "🔵" if row['Sistema'] == "Eproc" else "⚫"
                    sis_class = "header-eproc" if row['Sistema'] == "Eproc" else "header-legado"
                    
                    # LABEL BASE
                    label = f"{sis_icon} {nome}"
                    
                    # DESTAQUE VISUAL NA FILA
                    if n_col == "Fila Bastão":
                        if row['Consultor'] == proximo_nome:
                            label = f"⏩ {sis_icon} {nome} (PRÓXIMO)"
                        elif row['Pulando']:
                            label = f"⏭️ {sis_icon} {nome} (PULANDO)"
                    
                    # DETALHES (Atividade, Projeto, etc)
                    if ":" in status_full:
