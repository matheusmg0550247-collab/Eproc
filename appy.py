...
def render_fireworks(): st.markdown("""<style>...</style>""", unsafe_allow_html=True) # Resumido para poupar espaço
...
# --- FUNÇÃO DE SANEAMENTO DIÁRIO (NOVA) ---
...
def toggle_queue(consultor):
    ...
    ensure_daily_reset()
    st.session_state.gif_warning = False; now_br = get_brazil_time()
    if consultor in st.session_state.bastao_queue:
        current_holder = next((c for c, s in st.session_state.status_texto.items() if 'Bastão' in s), None)
...
def leave_specific_status(consultor, status_type_to_remove):
    ensure_daily_reset()
    st.session_state.gif_warning = False; old_status = st.session_state.status_texto.get(consultor, '')
...
def enter_from_indisponivel(consultor):
    ...
    if now_hour >= 20 or now_hour < 6:
        st.toast("💤 Fora do expediente (20h às 06h)! Ação bloqueada.", icon="🌙")
        st.session_state[f'check_simples_Indisponível_{consultor}'] = False; time.sleep(1); st.rerun(); return
    ensure_daily_reset()
...
def rotate_bastao():
    ...
    else: st.warning('Não há próximo(a) consultor(a) elegível na fila no momento.'); check_and_assume_baton()
...
def update_status(new_status_part, force_exit_queue=False):
    ...
    # 1. Calcula Sucessor ANTES de sair da fila (Correção do Almoço)
    forced_successor = None
...
with col_principal:
    ...
    if st.session_state.active_view:
        with st.container(border=True):
            if st.session_state.active_view == 'menu_atividades':
                ...
                if st.button("Confirmar", type="primary", use_container_width=True): update_status(f"Projeto: {st.selectbox('Projeto:', OPCOES_PROJETOS)}"); st.session_state.active_view = None; st.rerun()
    ...
    if st.session_state.active_view == "checklist":
        ...
        # ... (Código da view de chamados mantido)
        ...
        with st.container(border=True):
            ...
            if st.button("Gerar", type="primary", use_container_width=True): st.success("Gerado!") # Lógica simplificada aqui para caber
