...
def classificar_categoria(row):
    ...
    # Se está na fila, aparece na Fila, independente de ter atividade
...
# --- RENDERIZAÇÃO DAS COLUNAS (COM CORES NO LABEL) ---
        ...
        with cols[i]:
            ...
            if n_col == "Bastão":
                ...
                if not filtrados.empty: 
                    ...
                    # O primeiro que NÃO estiver pulando é o Próximo.
            ...
            if filtrados.empty:
                ...
                for j, (idx_df, row) in enumerate(filtrados.iterrows()):
                    ...
                    # 1. DESTAQUES DE FILA COM CORES (Streamlit Markdown)
                    if n_col == "Fila Bastão":
                        if row['Consultor'] == proximo_nome:
                            label = f"⏩ {sis_icon} {nome} :green[(PRÓXIMO)]"
                        elif row['Pulando']:
                            label = f"⤵️ {sis_icon} {nome} :orange[(PULOU)]"
                    
                    # 2. DETALHES DE TEXTO (ATIVIDADE/PROJETO/SESSÃO)
                    if ":" in status_full:
