...
# Importações de utilitários (Mantendo seu utils.py original)
from utils import (get_brazil_time, get_secret, send_to_chat)

def render_dashboard(team_id, team_name, consultores_list, webhook_key, app_url):
    # ============================================
    # 1. CONFIGURAÇÕES E CONSTANTES (DINÂMICAS)
    # ============================================
    DB_APP_ID = team_id          # ID dinâmico vindo do app.py
    LOGMEIN_DB_ID = 1            # Mantido fixo conforme original
    CONSULTORES = sorted(consultores_list) # Lista dinâmica
    APP_URL_CLOUD = app_url

    # Listas de Opções (MANTIDAS EXATAMENTE IGUAIS)
    ...
    # Secrets (DINÂMICO)
    # Busca o secret baseado na chave passada pelo app.py (ex: "bastao_eq1" ou "bastao_eq2")
    ...
    # Removido cache daqui para evitar conflito entre equipes, carregando direto
    ...
    def get_browser_id():
        ...
        try:
            return st_javascript(js_code, key=f"browser_id_tag_{team_id}") # Key única por time
    ...
    def handle_chamado_submission():
         # Wrapper para usar o session state dentro da função se necessário
    ...
    def save_state():
        try:
            ...
            # INVALIDAÇÃO DE CACHE DEPOIS DE SALVAR
            # Não precisamos limpar tudo, apenas invalidar a leitura se necessário
            # load_state_from_db.clear() -> Função local não tem clear. Está ok.
    ...
    # 8. INTERFACE
    # ============================================
    # REMOVIDO st.set_page_config DAQUI pois deve ficar no app.py
    ...
    def render_header_info_left():
        ...
        with c_topo_dir:
            ...
            # Válvula de Escape - REMOVIDO load_state_from_db.clear() pois é função local
    ...
    with col_principal:
        ...
        # -------------------------------------------------------------
        # AQUI ESTÁ A CORREÇÃO DO ALMOÇO (SKIP + MANTER NA FILA)
        # -------------------------------------------------------------
        if r2c5.button('🍽️ Almoço', use_container_width=True):
            consultor_atual = st.session_state.get('consultor_selectbox')
            if consultor_atual and consultor_atual != 'Selecione um nome' and consultor_atual in st.session_state.bastao_queue:
                # Pula automaticamente
                st.session_state.skip_flags[consultor_atual] = True
            # Define status, NÃO remove da fila (False), MANTEM fila (True)
            update_status('Almoço', marcar_indisponivel=False, manter_fila_atual=True)
            st.rerun()
                ...
                with c2:
                    if st.button("💾 Salvar e Notificar", type="primary", use_container_width=True):
                        ...
                        if verificar_duplicidade_certidao(tipo_cert, c_proc, c_data): 
                            ...
                            payload = {
                                "tipo": tipo_cert, "data": c_data.isoformat(), "consultor": c_cons, 
                                "incidente": c_chamado, # Mapeia chamado -> incidente
                                "processo": c_proc, "motivo": c_motivo,
                                "nome_parte": c_nome_parte, "peticao": c_peticao
                            }
